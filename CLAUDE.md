...
## Project Memory (durable context & hard-won gotchas)

Carried over from the previous machine's assistant memory — the *why* behind decisions and traps that aren't visible in the code. Point-in-time notes; verify against current code before asserting as fact.

**Why it exists — the north star.** The owner's **daughter** used Apple Photo Booth constantly and misses it; the whole point is that "she opens it and it feels like **coming home**" — kid-simple, delightful, recognizably Photo Booth (live 3×3 grid, white-screen flash, 2×2 quad default, bottom reel). She is the **north-star user**; weight her direct requests heavily. Her named asks so far are **all delivered** as of `v2.5.0`: *red-not-pink* accent, *Dizzy Birds* (her #1), and *face props*.

**Visual identity — two flavors, both retro, different lineages (don't regress).**
- **PopStrip flavor = authentic 90s-web.** A *cartoony modern pastiche* (rounded corners, halftone dots, candy double-bevels, pop-pastel gradients, Verdana) was **explicitly rejected as kitsch**; so was an over-corrected "premium minimal" look. The locked direction: **sharp corners (`border-radius:0`)**, 2–3px black borders, **hard OFFSET shadows (no blur)**, flat saturated web-safe color — **brand red `#e5202b`** (swapped from pink `#ff2d8f` at the daughter's request; do **not** call the var `--magenta`), plus yellow `#ffd21e` / cyan `#12cfda` / blue `#2b34d6` / lime `#8fd92e` — chunky **Impact/Arial-Black** display + Arial UI + Courier for data, tiled desktop. "Composed" shadow depths, not exaggerated.
- **Photobooth flavor = "if Apple shipped Photo Booth on the iMac G3 (~1999)":** Aqua/Platinum candy — glossy gel buttons, pinstripe title bar, brushed-metal shelf, translucent candy cradle, **Bondi `#0095b6` `--candy` default** (the switchable iMac-color swatch picker is deferred; `--candy` is the single hook), Lucida Grande. Scoped in `app.css` via `.app[data-flavor='photobooth']`; the `border-radius:0` kill rule is deliberately narrowed to `:not([data-flavor='photobooth'])` so the Aqua soft corners win without `!important` fights.
- Both keep **light + dark**.

**Architecture decisions deliberately deferred (don't "fix" casually).**
- The **3 parallel effect paths have no shared abstraction on purpose.** Green-screen composites *post-effect, off the pixi path*; AR overlays and frames are *additive top layers*. A `pipeline.ts` convergence has been consciously deferred at every release — only attempt it one pixel-diffed step at a time, gated by pb-verify.
- The **478-pt MediaPipe Face Landmarker (~3.7 MB) is deliberately NOT used.** All AR (Dizzy Birds, Lovestruck, the 7 face props) is placed from the lightweight 6-keypoint **FaceDetector** anchor `{cx, cy, size, roll}` + canonical face proportions. The mesh is reserved for **face paint / face-conforming effects** if/when they're built — the only place it earns its size.

**pixi.js v8 / GPU shader gotchas (matter for any shader or video work).**
- **Verify shaders in a real WebGL context** (a throwaway `shadertest.html` that compiles each Filter) — `npm run check`/`build` do **not** catch GLSL compile/link errors.
- Filter fragment shaders must declare `uInputSize`/`uInputClamp` as `uniform highp vec4` to match pixi's default vertex shader; a precision mismatch fails to link **only where the uniform is actually used** (unused ones get optimized out, hiding the bug).
- `VideoSource.destroy()` **pauses and clears** the underlying `<video>` (`.pause(); .src=''; .load()`) — never destroy a live-camera texture. Bind **one** shared `<video>` and never rebind (device switch keeps the same element).
- Neighbor-sampling shaders (comic/glow) must **normalize their texel kernel to a 720-px reference**, or the capped-720 preview and native-res capture won't match (WYSIWYG break).
- Use a **bare `autoDetectRenderer`** (not `Application`) and drive `renderer.render()` yourself per rAF; set `VideoSource.updateFPS = 0` (rVFC-driven).

**Effect fidelity.** Photo Booth's effects **are Apple Core Image filters** (Bulge=`CIBumpDistortion`, Dent=bump-negative, Twirl=`CITwirlDistortion`, Squeeze=`CIPinchDistortion`, Light Tunnel=`CILightTunnel`, Comic=`CIComicEffect`, Sepia=`CISepiaTone`, B&W=`CIPhotoEffectMono`) — replicate their math for accuracy. The owner wants final accuracy **locked by the automated `tools/pb-verify` harness, not a manual eyeball pass.** Aspect is **4:3** (Photo Booth is landscape — 4:3 classic / ~3:2 modern; the initial "square" belief was wrong). Open questions the Mac capture will settle: Dent bump-vs-pinch, Squeeze radius, Light Tunnel ring-freeze-vs-fold, Pop Art single-vs-Warhol-tiles, exact aspect + pixel dims, 4-up inter-frame interval.

**Operational gotchas.**
- **Black screen right after a deploy → suspect a stale service-worker/cache on the user's normal browser first, not a code bug.** Decisive test: open it in **incognito** (fresh SW/cache). If incognito works, the deploy is fine and the fix is DevTools → Application → **Clear site data**. Content-hashed `/assets/*` + a network-first shell mean users get new code on the next navigation even without an SW bump; the cache-name bump just forces it.
- **The dev/preview browser has no camera** — verify logic/mechanisms headless (compose, `ctx.filter` bake, effect registry, encoders, the pb-verify renders); live-camera UI needs the owner's real webcam. Beware: a dynamic `import()` in a preview-eval can resolve a *different* module instance than the running app (dual-singleton), so forcing `camera.status='live'` from eval may not actually mount the booth — don't trust it.
